...
class nd_router_advert(stringify.StringifyMixin):
    ...
    def serialize(self):
        res = self.res << 6
        #modification perso : changing res2 to set Home Agent Field to 1
        res2 = res|32
        hdr = bytearray(struct.pack(
            nd_router_advert._PACK_STR, self.ch_l, res2, self.rou_l,
            self.rea_t, self.ret_t))
        print('FROM ICMPV6 MODULE : data serialization')
        print(repr(hdr))
