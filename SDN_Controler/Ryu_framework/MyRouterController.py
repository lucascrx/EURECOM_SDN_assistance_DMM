...
import ryu.ippackage.ipaddress
...
def rest_command(func):
    def _rest_command(*args, **kwargs):
        print('start rest command')
        try:
            msg = func(*args, **kwargs)
            print('rest command message ok')
...
class RouterController(ControllerBase):
    ...
    def packet_in_handler(cls, msg):
        print('!!!!!!!!!!!RouterControllerPacketIN:')
        print(msg)
...
class Router(dict):
    ...
    def set_data(self, vlan_id, param, waiters):
        ...
        for vlan_router in vlan_routers:
            try:
                print('Router Set DATA')
                msg = vlan_router.set_data(param)
                msgs.append(msg)
                print('Router Set DATA DONE')
                if msg[REST_RESULT] == REST_NG:
                    # Data setting is failure.
                    print('####FAILURE ROUTER SET DATA####')
                    self._del_vlan_router(vlan_router.vlan_id, waiters)
                print('ok for router set data')
            except ValueError as err_msg:
                # Data setting is failure.
                self._del_vlan_router(vlan_router.vlan_id, waiters)
                print('####router set data exeption')
                raise err_msg
    ...
    def packet_in_handler(self, msg):
        print('router packet in handler')
        print(msg)
    ...
    def _cyclic_update_routing_tbl(self):
        while True:
            ...
            for vlan_router in self.values():
                vlan_router.send_neighborDisc_all_gw()
                hub.sleep(1)
...
class VlanRouter(object):
    ...
    def set_data(self, data):
        ...
        try:
            ...
            if REST_ADDRESS in data:
                ...
                print("@ L608 _set_data : ")
                print(address)
                address_id = self._set_address_data(address)
                print('#####set address data DONE######')
    ...
    def _set_address_data(self, address):
        ...
        print("L730 _set_address_data")
        print(address)
        print("nw_addr")
        print(address.nw_addr)
        ...
        # Send GARP
        self.send_neighborDisc_request(address.default_gw, address.default_gw)

        return address.address_id

    def _set_routing_data(self, destination, gateway):
        err_msg = 'Invalid [%s] value.' % REST_GATEWAY
        dst_ip = ip_addr_pton(gateway, err_msg=err_msg)
        ...
        if address is None:
            ...
            self.send_neighborDisc_request(src_ip, dst_ip)
    ...
    def packet_in_handler(self, msg, header_list):
        ...
        if IPV6 in header_list:
            ...
            if header_list[IPV6].dst in rt_ports:
                # Packet to router's port.
                if ICMPV6 in header_list:
                    if header_list[ICMPV6].type == icmpv6.ND_NEIGHBOR_SOLICIT \
                            or header_list[ICMPV6].type == icmpv6.ND_NEIGHBOR_ADVERT:
                        self._packetin_NeighborDisc(msg, header_list)
                    elif header_list[ICMPV6].type == icmpv6.ICMPV6_ECHO_REQUEST:
                        self._packetin_icmpv6_req(msg, header_list)
                        return
    ...
    def _packetin_NeighborDisc(self, msg, header_list):
        src_addr = self.address_data.get_data(ip=header_list[ICMPV6].src_ip)
        if src_addr is None:
            return
        ...
        src_ip = header_list[ICMPV6].src_ip
        dst_ip = header_list[ICMPV6].dst_ip
        ...
        if src_ip == dst_ip:
            ...
            self.logger.info('Receive GICMPV6 from [%s].', srcip,
                             extra=self.sw_id)
            self.logger.info('Send GICMPV6 (normal).', extra=self.sw_id)
            ...
            if (dst_addr is not None and
                ...
                self.logger.info('Receive ICMPV6 from an internal host [%s].',
                                 srcip, extra=self.sw_id)
                self.logger.info('Send ICMPV6 (normal)', extra=self.sw_id)
        else:
            if header_list[ICMPV6].opcode == icmpv6.ND_NEIGHBOR_SOLICIT:
                # ARP request to router port -> send ARP reply
                src_mac = header_list[ICMPV6].src_mac
                dst_mac = self.port_data[in_port].mac
                target_mac = dst_mac
                output = in_port
                in_port = self.ofctl.dp.ofproto.OFPP_CONTROLLER

                self.ofctl.send_neighDisc('S', self.vlan_id,
                                    dst_mac, src_mac, dst_ip, src_ip,
                                    target_mac, in_port, output)

                log_msg = 'Receive ICMPV6 NDISC request from [%s] to router port [%s].'
                self.logger.info(log_msg, srcip, dstip, extra=self.sw_id)
                self.logger.info('Send ICMPV6 reply to [%s]', srcip,
                                 extra=self.sw_id)

            elif header_list[ICMPV6].opcode == icmpv6.ND_NEIGHBOR_ADVERT:
                #  ARP reply to router port -> suspend packets forward
                log_msg = 'Receive ICMPV6 NADV reply from [%s] to router port [%s].'
                self.logger.info(log_msg, srcip, dstip, extra=self.sw_id)
    ...
    def _packetin_icmpv6_req(self, msg, header_list):
        # Send ICMP echo reply.
        in_port = self.ofctl.get_packetin_inport(msg)
        self.ofctl.send_icmpv6(in_port, header_list, self.vlan_id,
                             icmpv6.ICMPV6_ECHO_REPLY,
                             icmpv6.ICMPV6_ECHO_REPLY,
                             icmp_data=header_list[ICMPV6].data)

        srcip = ip_addr_ntoa(header_list[IPV6].src)
        dstip = ip_addr_ntoa(header_list[IPV6].dst)
    ...
    def _packetin_to_node(self, msg, header_list):
        ...
        if src_ip is not None:
            ...
            self.send_neighborDisc_request(src_ip, dst_ip, in_port=in_port)
    ...
    def send_neighborDisc_all_gw(self):
        gateways = self.routing_tbl.get_gateways()
        for gateway in gateways:
            address = self.address_data.get_data(ip=gateway)
            self.send_neighborDisc_request(address.default_gw, gateway)

    def send_neighborDisc_request(self, src_ip, dst_ip, in_port=None):
        ...
        for send_port in self.port_data.values():
            if in_port is None or in_port != send_port.port_no:
                ...
                target_mac = mac_lib.DONTCARE_STR
                inport = self.ofctl.dp.ofproto.OFPP_CONTROLLER
                output = send_port.port_no
               # discMessage = icmpv6.nd_neighbor(res='R', dst=dst_ip, option=None)

                self.ofctl.send_neighDisc('R', self.vlan_id,
                                    src_mac, dst_mac, src_ip, dst_ip,
                                    target_mac, inport, output)
...
class AddressData(dict):
    ...
    def add(self, address):
        ...
        nw_addr, mask, default_gw = nw_addr_pton(address, err_msg=err_msg)

        # Check overlaps
        # print('overlaps chacking...')
        # for other in self.values():
        #     print('previous values to chack')
        #     print(other)
        #     other_mask = mask_ntob(other.netmask)
        #     add_mask = mask_ntob(mask, err_msg=err_msg)
        #     if (other.nw_addr == ipv6_apply_mask(default_gw, other.netmask) or
        #             nw_addr == ipv6_apply_mask(other.default_gw, mask,
        #                                        err_msg)):
    ...
    def get_data(self, addr_id=None, ip=None):
        for address in self.values():
            if addr_id is not None:
                ...
                if ipv6_apply_mask(ip, address.netmask) == address.nw_addr:
                    return address
        return None
...
class Address(object):
    ...
    def __contains__(self, ip):
        return bool(ipv6_apply_mask(ip, self.netmask) == self.nw_addr)
...
class RoutingTable(dict):
    ...
    def add(self, dst_nw_addr, gateway_ip):
        ...
        if dst_nw_addr == DEFAULT_ROUTE:
            ...
            dst_ip, netmask, dummy = nw_addr_pton(
                dst_nw_addr, err_msg=err_msg % REST_DESTINATION)

        gateway_ip = ip_addr_pton(gateway_ip, err_msg=err_msg % REST_GATEWAY)
    ...
    def get_data(self, gw_mac=None, dst_ip=None):
        if gw_mac is not None:
            ...
            mask = 0
            for route in self.values():
                if ipv6_apply_mask(dst_ip, route.netmask) == route.dst_ip:
                    # For longest match
...
class SuspendPacketList(list):
    ...
    def wait_nbDisc_reply_timer(self, suspend_pkt):
...
class SuspendPacket(object):
    def __init__(self, in_port, header_list, data, timer):
        ...
        self.dst_ip = header_list[IPV6].dst
...
class OfCtl(object):
    ...
    def send_neighDisc(self, nd_fields, vlan_id, src_mac, dst_mac,
                 src_ip, dst_ip, target_mac, in_port, output):
        # Generate NEIGHBOR DISCOVERY packet
        if vlan_id != VLANID_NONE:
        ...
        a = icmpv6.nd_neighbor(res=nd_fields, dst=dst_ip, option=None)
        pkt.add_protocol(e)
        if vlan_id != VLANID_NONE:
            pkt.add_protocol(v)
        pkt.add_protocol(a)
    ...
    def send_icmpv6_other(self, in_port, protocol_list, vlan_id, icmp_type,
        ...
        if icmp_data is None and msg_data is not None:
            ...
            if icmp_type == icmpv6.ICMPV6_DST_UNREACH:
                icmp_data = None #icmpv6.dest_unreach(data_len=len(ip_datagram),
                                              #data=ip_datagram)
            elif icmp_type == icmpv6.ICMPV6_TIME_EXCEEDED:
                icmp_data = None #icmpv6.TimeExceeded(data_len=len(ip_datagram),
                                              #data=ip_datagram)

        ic = icmpv6.icmpv6(icmp_type, icmp_code, csum, data=icmp_data)

        ip = protocol_list[IPV6]
        if src_ip is None:
            src_ip = ip.dst
        ip_total_length = ip.header_length * 4 + ic._MIN_LEN
        # if ic.data is not None:s
        #     ip_total_length += ic.data._MIN_LEN
        #     if ic.data.data is not None:
        #         ip_total_length += + len(ic.data.data)
        # i = ipv6.ipv6(ip.version, ip.header_length, ip.tos,
        #               ip_total_length, ip.identification, ip.flags,
        #               ip.offset, DEFAULT_TTL, inet.IPPROTO_ICMP, csum,
        #               src_ip, ip.src)
        i = ipv6.ipv6(version=ip.version,payload_length=ip_total_length-ip.header_length,dst=ip.src,src=src_ip)
        pkt = packet.Packet()
        pkt.add_protocol(e)
        if vlan_id != VLANID_NONE:
            pkt.add_protocol(v)
...
class OfCtl_v1_0(OfCtl):
    ...
    def set_flow(self, cookie, priority, dl_type=0, dl_dst=0, dl_vlan=0,
        ...
        if nw_src:
            v = (128 - src_mask) << ofp.OFPFW_NW_SRC_SHIFT | \
                ~ofp.OFPFW_NW_SRC_MASK
            wildcards &= v
            nw_src = ipv6_text_to_int(nw_src)
        if nw_dst:
            v = (128 - dst_mask) << ofp.OFPFW_NW_DST_SHIFT | \
                ~ofp.OFPFW_NW_DST_MASK
            wildcards &= v
            nw_dst = ipv6_text_to_int(nw_dst)
        if nw_proto:
...
class OfCtl_after_v1_2(OfCtl):
    ...
    def set_flow(self, cookie, priority, dl_type=0, dl_dst=0, dl_vlan=0,
        ...
        if nw_src:
            nw_src_array = [int(x, 16) for x in nw_src.split(':')]
            mask_hex = hex(mask_ntob(src_mask))[2:-1]
            mask_array = [int(mask_hex[i:i+4],16) for i in range(0,len(mask_hex),4)]
            print('#######SRC1#######')
            match.set_ipv6_src_masked(nw_src_array,
                                      mask_array)
            print('#######SRC2#######')
        ...
        if nw_dst:
            print(nw_dst)
            print(dst_mask)
            nw_dst_array = [int(x, 16) for x in nw_dst.split(':')]
            mask_hex = hex(mask_ntob(dst_mask))[2:-1]
            mask_array = [int(mask_hex[i:i+4],16) for i in range(0,len(mask_hex),4)]
            print('#######DST1#######')
            match.set_ipv6_dst_masked(nw_dst_array,
                                      mask_array)
            print('#######DST2#######')
...
def ip_addr_pton(ip_str, err_msg=None):
    print('IP ADDR PTON')
    print(ip_str)
    try:
        return addrconv.ipv6.bin_to_text(socket.inet_pton(socket.AF_INET6, ip_str))
...
def ip_addr_ntoa(ip):
    return socket.inet_ntop(socket.AF_INET6,addrconv.ipv6.text_to_bin(ip))


def mask_ntob(mask, err_msg=None):
    try:
        print("MASK")
        print ((UINT128_MAX << (128 - mask)) & UINT128_MAX)
        return (UINT128_MAX << (128 - mask)) & UINT128_MAX
    except ValueError:
        msg = 'illegal netmask'
        if err_msg is not None:
            msg = '%s %s' % (err_msg, msg)
...
def ipv6_apply_mask(address, prefix_len, err_msg=None):
    import itertools

    assert isinstance(address, str)
    address_int = ipv6_text_to_int(address)
    return ipv6_int_to_text(address_int & mask_ntob(prefix_len, err_msg))


def ipv6_int_to_text(ip_int):
    assert isinstance(ip_int, (int, long))
    n = ip_int
    a = n >> 64
    b = n & (( 1  << 64) - 1) 
    return addrconv.ipv6.bin_to_text(struct.pack('!2Q', a, b))
...
def ipv6_text_to_int(ip_text):
    ...
    #return struct.unpack('!2Q', addrconv.ipv6.text_to_bin(ip_text))[0]
    c , d =  struct.unpack('!2Q', addrconv.ipv6.text_to_bin(ip_text))
    resultint = ( c << 64 ) | d
    result = resultint
    print ("IPV6 TEXT TO INT" )
    print( (c << 64 ) | d )
    print ("-----coucou-------")
    print resultint
    print result
...
def nw_addr_pton(nw_addr, err_msg=None):
    print('nw_addr_pton')
    print(nw_addr)
    ip_mask = nw_addr.split('/')
    default_route = ip_addr_pton(ip_mask[0], err_msg=err_msg)
    netmask = 32
    if len(ip_mask) == 2:
        try:
    ...
    if netmask < 0:
        ...
        if err_msg is not None:
            msg = '%s %s' % (err_msg, msg)
        raise ValueError(msg)
    nw_addr = ipv6_apply_mask(default_route, netmask, err_msg)
    print('output')
    print(nw_addr)
    #MY MODIFICATION
    return ip_mask[0], netmask, ip_mask[0]
